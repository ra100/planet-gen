...
| 4.5.6 | Continental structure: add low-frequency terrain base layer per research section 7.3. Use Voronoi cells or very low-frequency fBm (freq ≈ 2-4) to create continent-scale landmasses before adding detail fBm on top. Plate tectonics flag: active → distinct continents with ocean basins (bimodal elevation like Earth); stagnant lid → unimodal (like Venus/Mars). This replaces the current all-noise terrain that produces only small noisy islands | Test: Earth-like params produce 3-7 visible continent-scale landmasses separated by ocean; Mars-like produces highlands/lowlands dichotomy; changing seed produces different continent shapes | Phase 4 | cc:TODO |
