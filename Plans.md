...
| U15 size/anvil/organization gates | 15 (was 21 at FE-081; per-seed triage in the RV-003 section below) compliant-anvil / shear-driven-plume gate failures across the frozen seeds remain known-blocked after FE-081 de-classification | **Triage complete 2026-09-05** (`docs/research/2026-09-05-u15-triage-findings.md`): reproduction confirmed (8 plume + 6 anvil + 1 org); bisection pins the ws=1 fat-blob regression to `0e2b044` (static inland humidity source removed → one-shot coastal dump instead of transport-limited streaks; eddy diffusion aggravates, does not cause). Three Option B levers measured and rejected (provenance-keyed floor is a no-op — production runs without provenance; self-referential floor dilution-limited; absolute 0.16 q_sat land floor regresses sharpness/elongation, reverted). Awaiting user ruling on Option C gate surgery: C-1 axis gate → plume2-only + ws=1 downwind-organization check; C-2 L2/L1 band [1.50,2.50] → keep-and-park 4 seeds or re-specify [1.10,2.75]; C-3 sharpness keep (seed 7 marginal 0.735); P3 organization stays parked (known-blocked since FE-034–047). Net if approved: 15 → 7 (all organization-related). Do not mark green until ruled. |
