...
| U15 size/anvil/organization gates | 15 (was 21 at FE-081; per-seed triage in the RV-003 section below) compliant-anvil / shear-driven-plume gate failures across the frozen seeds remain known-blocked after FE-081 de-classification | **Triage complete + user-approved gate surgery landed 2026-09-05** (`docs/research/2026-09-05-u15-triage-findings.md`): reproduction confirmed (8 plume + 6 anvil + 1 org); bisection pins the ws=1 fat-blob regression to `0e2b044` (static inland humidity source removed → one-shot coastal dump instead of transport-limited streaks; eddy diffusion aggravates, does not cause). Three Option B levers measured and rejected. User rulings: C-1 axis ≤30° → plume2-only + ws=1 downwind-organization check (centroid ≥ +8 texels downwind, upwind mass ≤ 5% — measured +33…+36 texels / 0%); C-2 L2/L1 [1.50,2.50] → [1.10,2.75]; C-3 S2/S1 lower bound 0.75 → 0.70 (user override). **Total failures 15 → 8** = 6 anvil + 1 org count (2/8) + 1 disclosed marginal (seed 19 plume2 axis 34.8°, flagged in the proposal, parked per never-force-green). **P3 follow-up (2026-09-05): the FE-034–047 "upwind anvil centroid" was a metric bug** — `u15_compliant_anvil_metrics` aggregated all sphere-wide deep cores into one phantom centroid and measured downwind/PCA there; per-core measurement shows every core drifting **+10…+17 texels downwind** (20–35× the +0.5 requirement). User ruled **C-4: adopt the per-core metric** (identical thresholds; capture = 3× core radius, nearest-core assignment; findings §6) — anvil failures now 5 (19/101/211/509/997, all single-core PCA-angle outliers 21.6–49.5° on genuinely elongated anvils), org **3/8** (7/37/73 pass; 509/997 honestly regress from phantom-pass), total **8 → 7**. Tilt investigation complete (findings §6a) resolved into **C-5 (user-ruled 2026-09-05)**: the C-4 gate sampled wind *at the centroid*, folding in each core's own ×8 convergence inflow (storm-relative), which rotates "downwind" toward the centroid-offset direction — measured deflection equals the gate angle on all 27 cores. Re-measuring against the **environmental steering flow** (base flow + neighbor convergence, own-core excluded; meteorological convention) drops worst per-core tilt 49.5°→**5.8°**, min drift **+15.3 texels**; anvil failures 5→**0**, org **8/8**, total **7 → 1**. Thresholds unchanged, no shader change, A-gates bit-identical, 288 tests green (findings §6b). Remaining known-blocked: seed-19 plume2 axis 34.8° (unrelated shear-plume gate, parked per C-1). |
...
| U3 | Viewport state in file (v2): view_mode, rot, zoom, pan saved/restored as the user left the viewport | cc:完了 [d72e838] |
