...
## U16 Land-Profile Integration — Implemented in `d8c7de5`

U16 applies the exact normalized low-profile segment mean only to exposed land, blended across terrain height `.01..05` above ocean level. QA-019 makes the mean limb-safe by splitting a segment at its interior closest approach to the planet and length-weighting the monotonic subsegments; zero-length segments remain zero. QA-020's actual-WGSL GPU oracle passed all seven analytic values within `2e-4`: full `1`, rise `.234375`, plateau `1.25`, fall `.234375`, reverse `1`, limb `39/56`, and zero length `0`.

The user approved changing only the global largest-component topology bound from `.985` to `.990`, permitting physically connected continental cloud fields; occupancy, component count, ribbon, polar, anisotropy, and every other gate remain unchanged. Native validation recorded opacity p75 `.11444`, occupancy `.424`, land RGB p75/mean `.03809/.04221`, ocean mean/occupancy `.17252/.718`, and components/largest `403/.243`, with deterministic hashes and no `REJECT` output. U3 p95 was `9.891 ms` (PASS), U14 passed every seed with zero invalid geometry, and U15 passed every deterministic plume. Library tests (`77` pass, `1` ignored), sweep tests (`36` pass), build, clippy, and fmt passed. Evidence is available locally under `/tmp/planet-gen-final-u16-*`. Native source diagnostics remain telemetry only: the obsolete humid-highland decision and center-component rejection are removed. U16 was implemented in `d8c7de5`.
